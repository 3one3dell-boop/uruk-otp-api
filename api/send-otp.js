...
const { getAdmin, applyCors, validE164, rateLimit, readJson, clientIp, credFingerprint } = require("./_lib");
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // تفاصيل الخطأ الدقيقة + بصمة بيانات الاعتماد (آمنة، بدون كشف القيم السرّية) —
    // نشيلها بعد ما تنحل المشكلة
    return res.status(500).json({ error: "internal_error", detail: e.message || String(e), code: e.code || null, cred: credFingerprint() });
  }
};
